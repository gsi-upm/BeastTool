...
public abstract class MockAgentPlan extends Plan
{
	...
	protected String sendRequestToDF(String df_service, Object msgContent) {
		...
		if(receivers.length>0){
			...
			for(int i=0; i<receivers.length; i++){
				mevent.getParameterSet(SFipa.RECEIVERS).addValue(receivers[i].getName());
				logger.info("The receiver is "+receivers[i].getName());
			}
			sendMessage(mevent);
		}	
		logger.info("Message sended to "+df_service+" to "+receivers.length+" receivers");	
	 ...
	 * Given the IDFComponentDescription, the name of the agent is known using getName()
	 * 
	 * @param target_service The name of the df_service
	...
	protected IDFComponentDescription[] getReceivers(String target_service) {
		...
		IDFServiceDescription sd = df.createDFServiceDescription(target_service, null, null);
		...
		return (IDFComponentDescription[])ft.getParameterSet("result").getValues();
	}
