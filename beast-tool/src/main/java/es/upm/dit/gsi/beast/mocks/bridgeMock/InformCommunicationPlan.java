...
public class InformCommunicationPlan extends MockAgentPlan{
	...
	public void body()
	{
		...
		try {	agent_name = (String) ((IComponentIdentifier) actReq.getParameter(SFipa.SENDER).getValue()).getLocalName();	}
		catch (Exception e) {	System.out.println("Received message has no sender");}
		Object in_content = actReq.getParameter(SFipa.CONTENT).getValue();
		logger.info("IN: Type "+type+" ---- Sender_name: "+agent_name+" ---- Content: "+in_content);
		
		//The mock where the action to perform is saved
		AgentBehaviour behaviour = (AgentBehaviour) getBeliefbase().getBelief("agent_behaviour").getFact();
		...
		if (agent_name==null){
			...
			df_service =(String) behaviour.processMessage(type, agent_name, in_content);
			msgType = (String) behaviour.processMessage(type, agent_name, in_content);
			out_content = behaviour.processMessage(type, agent_name, in_content);
		}		
		logger.info("OUT: DF-Service "+ df_service+" ---- MsgType "+msgType+" ---- Content " +out_content);
				
		if (msgType=="SFipa.REQUEST") sendRequestToDF(df_service, out_content);
		if (msgType=="SFipa.INFORM")sendInformToDF(df_service, out_content);
	}	
	
//	/**
//	 * This method is an skeleton for future code-changes.
//	 * The idea is to process the arriving information (in_content) and create the output (out_content),
...
//	protected Object sendRequestAndGetResponse(Object request,IComponentIdentifier receiver) throws GoalFailureException {
