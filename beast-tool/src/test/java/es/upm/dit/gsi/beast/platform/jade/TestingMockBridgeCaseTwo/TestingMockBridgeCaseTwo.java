package es.upm.dit.gsi.beast.platform.jade.TestingMockBridgeCaseTwo;
...
public class TestingMockBridgeCaseTwo extends BeastTestCase {
    ...
    public Logger logger = Logger.getLogger(TestingMockBridgeCaseTwo.class
            .getName());
    ...
    public TestingMockBridgeCaseTwo() {
        ...
        try {
            ...
            LogActivator.logToFile(logger,
                    TestingMockBridgeCaseTwo.class.getName(), Level.ALL);
...
//         }
    ...
    public void createScenario(String scenarioName) {
        ...
        if (scenarioName
                .equals("one repository mock and one bridge mock in Jade Platform")) {
            startPlatform("jade", logger);
        } else {
            logger.severe("WARNING: "
                    + scenarioName
                    + " does not coincide with one repository mock and one bridge mock in Jade Platform");
        }
    }
    ...
    public void configureScenario(String setupName) {
        ...
        if (setupName
                .equals("bridge mocks has to send a message to repository")) {
            setScenario();
        } else {
            logger.severe("WARNING: "
                    + setupName
                    + " does not coincide with bridge mocks has to send a message to repository");
        }
    }
    ...
    public void checkScenario(String evaluationName) {
        ...
        if (evaluationName.equals("bridge receives the answer from repository")) {
            ...
            logger.severe("WARNING: "
                    + evaluationName
                    + " does not coincide with bridge receives the answer from repository");
        }
    }
