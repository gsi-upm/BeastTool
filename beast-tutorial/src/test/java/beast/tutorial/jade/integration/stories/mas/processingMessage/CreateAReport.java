package beast.tutorial.jade.integration.stories.mas.processingMessage;
 ...
 * 
 * 
 * This "AgentStory" is described as follows: Story: Processing message As a
 * ReporterAgent, I want to process any FIPA-INFORM message from a RecorderAgent
 * which contains a recorded message, So that I am able to generate an issue
 * report based on that message content.
 * 
 * This specific scenario is described as follows: Scenario: CreateAReport Given
 * a ReporterAgent has access to the database, and a RecorderAgent has received
 * a call that can understand, When that ReporterAgent receives a FIPA-INFORM
 * message from that RecorderAgent, Then the message is processed and a new
...
public class CreateAReport extends BeastTestCase {
	...
	public CreateAReport() {
		...
		try {
			...
			LogActivator.logToFile(logger, CreateAReport.class.getName(),
					Level.ALL);
	 ...
	 */
	public void verify() {
		checkAgentsBeliefEquealsTo("ReporterAgentUnderTesting",
				"createdIssueReport", true);
	}
	...
	public void createScenario(String scenarioName) {
		...
		if (scenarioName
				.equals("a ReporterAgent has access to the database, and a RecorderAgent has received a call that can understand,")) {
			startPlatform("jade", logger);
		} else {
			logger.severe("WARNING: "
					+ scenarioName
					+ " does not coincide with a ReporterAgent has access to the database, and a RecorderAgent has received a call that can understand,");
		}
	}
	...
	public void configureScenario(String setupName) {
		...
		if (setupName
				.equals("that ReporterAgent receives a FIPA-INFORM message from that RecorderAgent,")) {
			setScenario();
		} else {
			logger.severe("WARNING: "
					+ setupName
					+ " does not coincide with that ReporterAgent receives a FIPA-INFORM message from that RecorderAgent,");
		}
	}
	...
	public void checkScenario(String evaluationName) {
		...
		if (evaluationName
				.equals("the message is processed and a new issue report is created.")) {
			this.setExecutionTime(BeastTestCase.SLEEP_TIME);
			verify();
		} else {
			logger.severe("WARNING: "
					+ evaluationName
					+ " does not coincide with the message is processed and a new issue report is created.");
		}
	}
