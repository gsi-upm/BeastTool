package beast.tutorial.jade.integration.stories.mas.passingAnIncomingCall;
 ...
 * 
 * 
 * This "AgentStory" is described as follows: Story: Passing an incoming call As
 * a HelpDeskAgent, I want to process a FIPA-REQUEST message from a
 * RecorderAgent, So that I am able to pass an incoming call to a human
 * operator.
 * 
 * This specific scenario is described as follows: Scenario: PassACall Given a
 * HelpDeskAgent has connection to the desktop of the operator computer, and a
 * RecorderAgent has received a call that cannot understand, When that
 * HelpDeskAgent receives a FIPA-REQUEST message from that RecorderAgent, Then
 * the request is accepted and the incoming call is transfered to the human
 * operator.
 * 
...
public class PassACall extends BeastTestCase {
	...
	public PassACall() {
		...
		try {
			...
			LogActivator
					.logToFile(logger, PassACall.class.getName(), Level.ALL);
	...
	public void setup() {
		...
		// HelpDeskAgent under testing
		startAgent("HelpDeskAgentUnderTesting",
				"beast.tutorial.jade.agent.HelpDeskAgent");
	...
	public void verify() {
		...
		checkAgentsBeliefEquealsTo("HelpDeskAgentUnderTesting",
	...
	public void createScenario(String scenarioName) {
		...
		if (scenarioName
				.equals("a HelpDeskAgent has connection to the desktop of the operator computer, and a RecorderAgent has received a call that cannot understand,")) {
			startPlatform("jade", logger);
		} else {
			logger.severe("WARNING: "
					+ scenarioName
					+ " does not coincide with a HelpDeskAgent has connection to the desktop of the operator computer, and a RecorderAgent has received a call that cannot understand,");
		}
	}
	...
	public void configureScenario(String setupName) {
		...
		if (setupName
				.equals("that HelpDeskAgent receives a FIPA-REQUEST message from that RecorderAgent,")) {
			setScenario();
		} else {
			logger.severe("WARNING: "
					+ setupName
					+ " does not coincide with that HelpDeskAgent receives a FIPA-REQUEST message from that RecorderAgent,");
		}
	}
	...
	public void checkScenario(String evaluationName) {
		...
		if (evaluationName
				.equals("the request is accepted and the incoming call is transfered to the human operator.")) {
			this.setExecutionTime(BeastTestCase.SLEEP_TIME);
			verify();
		} else {
			logger.severe("WARNING: "
					+ evaluationName
					+ " does not coincide with the request is accepted and the incoming call is transfered to the human operator.");
		}
	}
